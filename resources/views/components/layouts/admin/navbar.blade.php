@php
  $links = [
    ...
    [
      'name' => 'Fundings',
      'href' => route('admin.funding.index'),
      'icon' =>
      ...
      'active' => request()->routeIs('admin.funding.index'),
    ],
    [
      'name' => 'Blogs',
    ...
    <nav class="flex h-full flex-col justify-between px-4 py-6">
      ...
      <div class="border-t border-gray-200 pt-6 dark:border-gray-700">
        <form action="{{ route('admin.logout') }}" method="POST">
          @csrf
          <button
            class="group flex w-full items-center space-x-3 rounded-xl px-4 py-3 text-red-600 transition-all duration-200 hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer"
            type="submit">
            <svg class="h-5 w-5 transition-transform group-hover:scale-110" fill="none"
              stroke="currentColor" viewBox="0 0 24 24">
              <path
